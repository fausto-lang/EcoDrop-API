...
def contabilidad_diaria(request, user_id):
    try:
        ...
        hoy = date.today()
        contabilidad_hoy = contabilidad.objects.filter(usuario=usuario, fecha=hoy)
        total_kilos=0.0
        total_ganancia=0.0
        separado_residuos={}
...
def ranking_recicladores(request):
    ...
    for usuario in ususarios:
        total_kilos=contabilidad.objects.filter(usuario=usuario).aggregate(Sum('kilos'))['kilos__sum'] or 0
        ranking.append({
...
def estadisticas_grafico(request):
    hoy=date.today()
    total_kilos=contabilidad.objects.filter(fecha=hoy).aggregate(Sum('kilos'))['kilos__sum'] or 0
    total_ganancia=contabilidad.objects.aggregate(Sum('ganancia_obtenida'))['ganancia_obtenida__sum'] or 0

    kilos_hoy=contabilidad.objects.filter(fecha=hoy).aggregate(Sum('kilos'))['kilos__sum'] or 0 
    kilos_mes=contabilidad.objects.filter(fecha__month=hoy.month).aggregate(Sum('kilos'))['kilos__sum'] or 0

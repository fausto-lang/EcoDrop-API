...
class AsistenciaInteligenteService:
    ...
    def clasificar_por_imagen(ruta_image_local) -> dict:
        ...
        prompt = (
            ...
            f"1. Identifica cuál de las siguientes categorías corresponde: [{opciones_explicitas}]. "
            "Si se trata de algo parecido (por ejemplo una botella de vidrio) asócialo al tipo más cercano tal cual está escrito, sin tildes ni modificaciones.\n"
            "Tu respuesta para 'categoria_detectada' debe ser estrictamente una de esas opciones, escrita exactamente igual.\n\n"
            "Responde ÚNICAMENTE un objeto JSON con la estructura exacta, sin bloques markdown:\n"
            '{"categoria_detectada": "nombre_de_la_categoria", "peso_estimado": 0.0}'
        )
        ...
        if resultado["estado"] == "success":
            try:
                ...
                if residuo_id:
                    ...
                    return {"error": f"La IA detectó '{ans_json.get('categoria_detectada')}', pero no coincide con ninguna categoría del sistema."}
            except Exception:
    ...
    def clasificar_por_voz(ruta_audio_local) -> dict:
        ...
        prompt = (
            ...
            "Si se trata de algo parecido asócialo al tipo más cercano tal cual está escrito, sin tildes ni modificaciones.\n"
            "Tu respuesta para 'tipo' debe ser estrictamente una de esas opciones, escrita exactamente igual.\n"
            "2. Extrae la cantidad numérica de peso mencionada en kilogramos. Si no se menciona, usa 0.0.\n\n"
            "Responde ÚNICAMENTE un objeto JSON con la estructura exacta, sin bloques markdown:\n"
            ...
            try:
                ...
                if residuo_id:
                    ...
                    try:
                        ...
                        respuesta_voz["cantidad_kg"] = 0.0
                    return respuesta_voz
                else:
                    return {"error": f"La IA detectó '{ans_json.get('tipo')}', pero no coincide con ninguna categoría del sistema."}
            except Exception:
                return {"error_parsing": "Error al parsear el JSON de la IA."}
        else:
            return {"error": f"Error en la IA: {resultado.get('error')}"}
